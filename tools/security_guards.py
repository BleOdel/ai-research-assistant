...
2. .gitignore — the personal-data ignore rules must all still be present.
   ...
   discovered sources or synthesis reports.
    ...
    for rule in REQUIRED_IGNORE_RULES:
        if rule not in rules:
            errors.append(
                ...
                "These rules keep fork users from committing their discovered sources or "
                "synthesis reports. If the rule moved or was renamed intentionally, update "
                "REQUIRED_IGNORE_RULES in tools/security_guards.py in the same PR."
            )
